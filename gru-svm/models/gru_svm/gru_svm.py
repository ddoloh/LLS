...
from models.gru_svm import llesmote
            ...
            with tf.name_scope('final_training_ops'):
                with tf.name_scope('weights'):
                    weight = tf.get_variable('weights',
                                             initializer=tf.random_normal([self.cell_size, self.num_classes], stddev=0.01))
                ...
                with tf.name_scope('llesmote'):
                    embeddings = tf.get_variable('embeddings', [self.batch_size, self.num_classes])
                    output = tf.nn.embedding_lookup(embeddings, tf.to_int32(output_[:,0]))
