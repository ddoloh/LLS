    ...
    def __init__(self, alpha, batch_size, cell_size, dropout_rate, num_classes, sequence_length, svm_c):
        ...
        def __graph__():
            ...
            with tf.name_scope('input'):
                ...
                x_onehot = tf.one_hot(indices=x_input, depth=self.cell_size, on_value=1.0, off_value=0.0, name='x_onehot')

                # [BATCH_SIZE]
                y_input = tf.placeholder(dtype=tf.uint8, shape=[None], name='y_input')	

                # [BATCH_SIZE, N_CLASSES]
                y_onehot = tf.one_hot(indices=y_input, depth=self.num_classes, on_value=1.0, off_value=0.0, name='y_onehot')
            ...
            with tf.name_scope('final_training_ops'):
                ...
                with tf.name_scope('Wx_plus_b'):
                    output_ = tf.matmul(last, weight) + bias
                    tf.summary.histogram('pre-activations', output_)

                
            with tf.name_scope('LLE-SMOTE'):
                clf = LocallyLinearEmbedding(4, n_components=2, method='standard', eigen_solver='dense')
                with tf.Session() as sesss:
                    output_ = output_.eval(session = sesss)
                output_lle = clf.fit_transform(output_)
                output_smote, _ = SMOTE().fit_resample(output_lle, y_input)
                output = tf.convert_to_tensor(output_smote, name="llesmote")
                tf.summary.histogram('lle-smote', output)


            # L2-SVM
            with tf.name_scope('svm'):
                regularization_loss = 0.3 * tf.reduce_sum(tf.square(weight))
                hinge_loss = tf.reduce_sum(tf.square(tf.maximum(tf.zeros([self.batch_size, self.num_classes]), 1 - y_onehot * output)))
            ...
            try:
                for step in range(epochs * train_size // self.batch_size):
                    ...
                    feed_dict = {self.x_input: train_example_batch, self.y_input: train_label_batch,
                                 self.state: current_state, self.learning_rate: self.alpha, self.p_keep: self.dropout_rate}

                    train_summary, _, predictions, actual, next_state = sess.run([self.merged, self.optimizer,
                                                                                  self.predicted_class, self.y_onehot,
                                                                                  self.states], feed_dict=feed_dict)
                ...
                for step in range(epochs * validation_size // self.batch_size):
                    ...
                    validation_summary, predictions, actual, validation_loss, validation_accuracy = \
                        sess.run([self.merged, self.predicted_class, self.y_onehot, self.loss, self.accuracy], feed_dict=feed_dict)
        ...
        with tf.Session() as sess:
            ...
            try:
                for step in range(test_size // batch_size):
                    ...
                    label_onehot = tf.one_hot(test_labels_batch, num_classes, 1.0, 0.0)
